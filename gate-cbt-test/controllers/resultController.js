...
exports.submitTest = async (req, res) => {
  try {
    ...
    const result = new Result({
      ...
      totalPossibleMarks,
      correct, wrong, attempted, skipped,
      rank, totalStudents, selected,
...
exports.downloadResultPdf = async (req, res) => {
  try {
    ...
    // Buffer mein collect karo — phir ek baar bhejo
    const buffers = [];
    ...
    // ── Student info ─────────────────────────────────────────────
    doc.rect(40, 115, W, 60).fill("#e0f2fe").stroke("#bae6fd");
    ...
    const rows = [
      ...
      ["Score %",  `${percentage.toFixed(1)}%`, "Cutoff", percentage >= 40 ? "Qualified" : "Not Qualified"],
    ];
    ...
    doc.end(); // ← Ye end hona zaroori hai — buffer trigger hoga
