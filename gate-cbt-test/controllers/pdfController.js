...
function parseAnswerMap(ansLines) {
  ...
  for (let line of ansLines) {
    const fmt1 = line.match(/^Q(\d+)\.\s+Answer:\s+([A-D]|-?\d+(?:\.\d+)?)\s+\[(MCQ|NAT)\s*\|\s*(\d+)\s*Marks?/i);
    if (fmt1) {
      const type = fmt1[3].toUpperCase(), marks = parseInt(fmt1[4]);
      answerMap[fmt1[1]] = { answer: type==="MCQ"?fmt1[2].toUpperCase():fmt1[2], type, marks, negativeMarks: type==="NAT"?0:marks===2?0.66:0.33 };
      continue;
    }
    let marks = 1;
    const mh = line.match(/\b([12])\s*(?:marks?|M)\b/i)||line.match(/[(\[]\s*([12])\s*[)\]]/);
    if (mh) marks = parseInt(mh[1]);
    const mcq = line.match(/^Q?(\d+)[.)]\s*(?:Ans(?:wer)?[:\s]*)?\(?([A-D])\)?/i)||line.match(/^Q?(\d+)\s+([A-D])$/i);
    if (mcq) { answerMap[mcq[1]]={answer:mcq[2].toUpperCase(),type:"MCQ",marks,negativeMarks:marks===2?0.66:0.33}; continue; }
    const nat = line.match(/^Q?(\d+)[.)]\s*(?:Ans(?:wer)?[:\s]*)?(-?\d+(?:\.\d+)?)$/i);
    if (nat) { answerMap[nat[1]]={answer:nat[2],type:"NAT",marks,negativeMarks:0}; }
  }
  return answerMap;
}
...
exports.uploadPDFs = async (req, res) => {
  try {
    ...
    const skipPatterns = [
      ...
      /^Not Attempted/i,/^—\s*END/i,/^All the best/i,/^Answer:\s*_+/i,/^\(Enter integer\)/i
    ];
    ...
    for (let line of pyqLines) {
      ...
      if (qMatch) {
        ...
        let type=answerInfo.type||"MCQ", marks=answerInfo.marks||1;
        let negativeMarks=answerInfo.negativeMarks??(marks===2?0.66:0.33);
        if (metaInline) {
          type=metaInline[1].toUpperCase(); marks=parseInt(metaInline[2]);
        ...
        if (inlineOpts) {
          questionsArray.push({ testId:test._id,questionNumber,question:inlineOpts[1].trim(),
            ...
            type:"MCQ",marks,negativeMarks:marks===2?0.66:0.33,year:testYear });
          currentQuestion = null;
        } else {
          currentQuestion = { testId:test._id,questionNumber,question:metaInline?"":restOfLine,
      ...
      if (currentQuestion) {
        ...
        if (metaMatch) {
          currentQuestion.type=metaMatch[1].toUpperCase(); currentQuestion.marks=parseInt(metaMatch[2]);
          currentQuestion.negativeMarks=/No Negative/i.test(line)?0:currentQuestion.marks===2?0.66:0.33;
          continue;
        }
        const optMatch = line.match(/^\(?([A-Da-d])\)?[.)]\s*(.+)/i);
        if (optMatch && currentQuestion.options.length<4) {
          currentQuestion.options.push(optMatch[2].trim()); currentQuestion.type="MCQ"; continue;
        }
        if (!skipPatterns.some(p=>p.test(line)) && line.length>3) {
          if (currentQuestion.question==="") currentQuestion.question=line;
          else if (currentQuestion.options.length===0) currentQuestion.question+=" "+line;
        }
      }
    }
    ...
    questionsArray.forEach(q => {
      ...
      if (!ansInfo) return;
      if (q.type==="NAT") { q.correctAnswer=String(ansInfo.answer); }
      else { const idx=ansInfo.answer.charCodeAt(0)-65; if(q.options[idx]!==undefined) q.correctAnswer=q.options[idx]; }
    });
    ...
    if (finalQuestions.length===0) {
      ...
      return res.status(400).json({ message:"No questions parsed. Check PDF format.",
        debug:{ answerMapKeys:Object.keys(answerMap), sampleLines:pyqLines.slice(0,15) } });
    }
    ...
    res.status(200).json({
      message:"PDF uploaded successfully", testId:test._id, testName:test.name,
